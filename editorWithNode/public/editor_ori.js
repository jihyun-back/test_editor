...
Editor.prototype.header = function () {
    ...
    if (headertd) {
        ...
        inputHidden.setAttribute('class', 'file');
        inputHidden.setAttribute('name', 'img');
...
Editor.prototype.headerEvent = function () {
    ...
    let newPageBtn = this.element.table.querySelector('.headerBtn');
...
Editor.prototype.imgUpload = function() {

    let fileUploadBtn = this.element.table.querySelector('.uploadBtn');

    fileUploadBtn.addEventListener('click', function(){
        this.element.table.querySelector('.file').click();
    }.bind(this,Editor))

    console.log(this.element.table.querySelector('.uploadForm'))
    let fileInput = this.element.table.querySelector('.file');
    fileInput.addEventListener('change', function(e) {
        
        const fd = new FormData();
        const inputData = this.element.table.querySelector('.file').files[0];
        fd.append('img',inputData);
        this.sendAjax('/up', fd);   
...
Editor.prototype.sendAjax = function(url, data) {
    ...
    req.onreadystatechange = function(){
        if(req.responseText){
            ...
            const img = JSON.parse(res).imgUrl;
            this.element.table.querySelector('.edit').innerHTML += `<p><img src = \"${img}\"><p>`
...
// deco 버튼 이벤트
Editor.prototype.decoBtn = function (aLink, decoList) {
    aLink.addEventListener('click', function () {
        this.element.table.querySelector('.edit').focus();
        document.execCommand(aLink.firstChild.className)
        if (document.queryCommandState(decoList)) {
            aLink.parentElement.style.backgroundColor = "#eee";
        } else {
            aLink.parentElement.style.backgroundColor = "white";
        }
    }.bind(this, Editor))
}
Editor.prototype.selectionDeco = function (decoChild, decoList) {


    document.addEventListener('selectionchange', function (e) {
        if (e.target.activeElement.parentElement.className === 'content') {

            let decoChild = e.target.activeElement.closest('.root').querySelector('.deco').childNodes;

            for (let i = 0; i < decoList.length; i++) {
                if (document.queryCommandState(decoList[i])) {
                    decoChild[i].style.backgroundColor = "#eee";
                } else {
                    decoChild[i].style.backgroundColor = "white";
                }
            }
        } else {
